...
const Input: React.FC<InputProps> = ({ 
  ...
  return (
    <View className={cn('w-full mb-4', containerClassName)}>
      {label && (
        <Text className="text-sm font-inter-medium text-muted mb-1.5 ml-1">
          {label}
        </Text>
      )}
      <View 
        className={cn(
          'flex-row items-center bg-surface border border-border rounded-3xl px-4 py-3.5',
          error && 'border-danger',
      ...
      >
        ...
        <TextInput
          className={cn(
            'flex-1 text-base text-text font-inter',
            className
          )}
